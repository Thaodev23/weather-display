...
var url = `https://api.openweathermap.org/data/2.5/weather?q=${currentCity}&appid=d873fc9a964d259941e1ffb84bcb79c6`;
var presentDate;
var temperature = 0;
var humidity = 0;
var icon = "";
...
var currentWeather = $("#present-weather");
var daysForecast = "";
var numDays = 1;
var arrayCity = [];
...
if (localStorageArray != null) {
  localStorageLength = localStorageArray.length;
  localStorageArray.forEach((iterCity) => {
    var currentCityButton = $(`<input type="button" class="currentCityButton" id="alpha" value='${iterCity}'>`);
    $("#search-block").append(currentCityButton);
  })
}

// function createButtonCities (currentCity) {
//   var currentCityButton = $(`<input type="button" class="currentCityButton" id="alpha" value="${currentCity}">`);
...
function weatherForecast () {
 ...
 .then(function (data) {
  ...
  <div id="text-on-top">
    <h2>${currentCity} ${presentDate} <img 
  ...
  currentWeather.append(weatherData);

  // makeBtn = $(`<button class = btn>
  //           ${currentCity}
  //           </button>`)

  // $("#search-block").append(makeBtn)
...
function fiveDaysCast (longitude, latitude) {
  ...
  .then(function (data) {
    ...
    for (var j = 0; j < 5; j++) {
      ...
      createButtonCities(currentCity);
  ...
  function createButtonCities (currentCity) {
    ...
    else if (localStorage.getItem("cities").includes(currentCity)) {
      console.log("return");
      return;
    }
    else { 
    ...
    localStorage.setItem("cities", JSON.stringify(arrayCity));
    
  }

  $(document).ready(function() {
    $(document).on("click", "input.currentCityButton", clickAppear);
  })
  ...
  function clickAppear(event) {
   ...
   FutureWeatherDis(longitude, latitude);
  ...
  function FutureWeatherDis(longitude, latitude) {
    getFiveDaysUrl = "https://api.openweathermap.org/data/2.5/forecast?lat=" + latitude + "&lon=" + longitude + "&appid=d873fc9a964d259941e1ffb84bcb79c6" 
